...
#else
        f->addFnAttr(Attribute::NoReturn);
        //klee_xqx_debug("func add NoReturn, f=%s", f->getNameStr().c_str());
#endif
      }
...
void SpecialFunctionHandler::handleCheckMemoryAccess(ExecutionState &state,
  ...
  std::ostringstream info;
	  ...
	  klee_xqx_debug("handleCheckMemoryAccess: address & size");
	  klee_xqx_debug("%s", info.str().c_str());
	  //address->dump();
	  //size->dump();
#endif
...
void SpecialFunctionHandler::handlePrintf(ExecutionState &state,
                                                     ...
                                                     std::vector<ref<Expr> > &arguments) {
  assert(arguments.size()>=1 &&
         "invalid number of arguments to printf");
  assert(isa<ConstantExpr>(arguments[0]) &&
         "expect constant address argument to printf");
  ...
  //FixMe, here we use the init string lenth, it is not match with the real one
  executor.bindLocal(target, state,
                     ConstantExpr::create(ret, Expr::Int32)); 
}
