...
namespace {
	...
	cl::opt<bool>
		UseConcolicSearcher("use-concolic-searcher",
				cl::init(false),
				cl::desc("use concolic searchers, such as ConcolicDFSSearcher"));

    cl::opt<bool>
		_enableFork("enable-fork",
				cl::init(true),
				cl::desc("set SE Forkable or not at beginning"));

    cl::opt<bool>
		forkOnlyFocusFunc("fork-in-focused-funcs",
				cl::init(false),
				cl::desc("set SE Forkable only in the focused functions"));

	cl::opt<bool>
		OutputFocusBBs("output-focus-bbs",
				cl::init(true),
                cl::desc("output bbs of focused functions to id-fbbs.txt"));

    cl::opt<bool>
		phaseTest("phase-test",
				cl::init(false),
				cl::desc("a switch for phase test"));
}
...
void Executor::branch(ExecutionState &state, 
    ...
    for (unsigned i=1; i<N; ++i) {
        xinfo << "fork switch condition[" << i << "] = \n" << conditions[i] << "\n" ;
    }
    klee_xqx_debug("%s in state[%d]",xinfo.str().c_str(), state.id);
#endif
    ...
    if( !enableFork ) {
        ...
        if (it != seedMap.end()) {
            ...
            assert(result.size()==1 && "branch error when switch in disablefork model");
            return;
        }
        else {
            klee_error("no seeds in disablefork model, when branch");
        }
...
Executor::fork(ExecutionState &current, ref<Expr> condition, bool isInternal) {
...
#if 1
#ifdef XQX_DEBUG
    //klee_xqx_debug("dump constraints in state[%d]. ", current.id);
    //printFileLine(current, current.pc);
    //current.constraints.dump();
	ref<Expr> simpCond = current.constraints.simplifyExpr(condition);
	if( ConstantExpr *CE = dyn_cast<ConstantExpr>(simpCond) ) {
        //std::ostringstream xinfo;
        //xinfo << "fork0 Constant condition = \n" << condition;
        //klee_xqx_debug("%s in state[%d]",xinfo.str().c_str(), current.id);
    }

	else {
        klee_xqx_debug("state[%d] with %s seed fork at :", current.id, isSeeding==true?"":"no");
...
#ifdef XQX_DEBUG
		klee_xqx_debug("solver=Unknown in !isSeeding");
#endif
	...
	if (res==Solver::True) {
		...
		//si->printSeedInfo();
          ...
          info << "condition is ----------\n" << condition << "\ncondition after evaluate:-----\n";
          info << tmp << "\n" ;
          klee_xqx_debug(info.str().c_str());
...
#if 1
        //select the concolic branch first when useConcreteData in DFSSearcher
        if(UseConcreteData && UseConcolicSearcher ){
            //std::swap(trueState, falseState);
            if(concolicBr) {
                //xaddedStates.insert(xaddedStates.end(), falseState);
                //xaddedStates.insert(xaddedStates.end(),trueState);
                searcher->setFlags(true);
                klee_xqx_debug("change to concolic state");
            } 
            //else {
                //xaddedStates.insert(xaddedStates.end(), trueState);
                //xaddedStates.insert(xaddedStates.end(), falseState);
...
#endif
#ifdef XQX_DEBUG_OFF
		klee_xqx_debug("fork in state[%d]------------", trueState->id);
		condition->dump();
		klee_xqx_debug("fork-create zero in state[%d]---------------", falseState->id);
...
#ifdef XQX_DEBUG
                klee_xqx_debug("seed patched violating, condition = ");
                condition->dump();
#endif
				warn = true;
			}
...
#define XQX_DUMP_ARGS
	...
	if( DumpFuncTrace )
	{
        ...
        double curCostTime = 0;

        if (startTime == 0) {
            startTime = util::getWallTime();
        } else {
            curCostTime = util::getWallTime() - startTime;
        }
		...
		klee_record_func("%s-%s -- %s:%d:%d  s[%d] [t:%f]" , info.str().c_str(), f->getName().str().c_str(), 
				filename.c_str(), ki->info->line, ki->info->assemblyLine, state.id, curCostTime);
#if 0
	...
	if (f && f->isDeclaration()) {
		...
		if( f->getName().str() == "read") {
			CheckMemoryOutofBound(state, arguments[1], arguments[2]);
		}
...
void Executor::printFileLine(ExecutionState &state, KInstruction *ki) {
	...
	if (ii.file != "") 
		std::cerr << "xloc: " << f->getName().str() << " - " << ii.file << ":" << ii.line << ":" <<ii.assemblyLine << "\n";
	else
		std::cerr << "loc: " << f->getName().str() << " - " << "     [no debug info]:"<<ii.assemblyLine << "\n";
}
		...
		case Instruction::Ret:
			{
...
#ifdef XQX_DEBUG_PHASE
                if( phaseTest && caller ) {
                    CallSite cs(caller);
                    Function *cf = dyn_cast<Function>(cs.getCalledValue());
                    if( cf ) {
                        klee_xqx_debug("func - %s called\n", cf->getNameStr().c_str());
                        if( cf->getNameStr().find("png_read_info")!=std::string::npos ){
                            klee_xqx_debug("okfunc - %s called\n", cf->getNameStr().c_str());
                            enableFork = true;
                        }
                    }
                }
				...
				if (state.stack.size() <= 1) {
					...
					klee_xqx_debug("exit-last-ret in state[%d], state.stacksize=%d",
                            state.id, state.stack.size());
...
#ifdef XQX_DEBUG_GETPTR
                std::ostringstream xinfo;
                xinfo << "getPtrElement: base = " << base;
                klee_xqx_debug(xinfo.str().c_str());
                printFileLine( state, ki);
                klee_xqx_debug("getPtrElement end");
#endif
...
void Executor::updateStates(ExecutionState *current) {
    //if( UseConcolicSearcher && searcher ) {
		//searcher->updateEx(current, xaddedStates, addedStates, removedStates);
        //xaddedStates.clear();
    //}
    if (searcher) {
...
void Executor::run(ExecutionState &initialState) {
	...
	while (!states.empty() && !haltExecution) {
		...
		KInstruction *ki = state.pc;
#ifdef XQX_FORKCHECK
        if( forkOnlyFocusFunc )
            enableFork = checkForkable(state);
#endif
		stepInstruction(state);
			...
			if ((stats::instructions & 0xFFFF) == 0) {
				...
				if (mbs > MaxMemory) {
					if (mbs > MaxMemory + 100) {
                        ...
                        unsigned maxMem = MaxMemory;
                        klee_xqx_debug("curMem=%d, MaxMem=%d", mbs, maxMem);
                        klee_xqx_debug("kill %d states in %d", toKill ,numStates);
                        if(toKill==numStates)
                            toKill = numStates / 2; //we kill half of all
...
std::string Executor::getAddressInfo(ExecutionState &state, 
		ref<Expr> address, ref<Expr> bytes) const{
	std::ostringstream info;
	std::pair< ref<Expr>, ref<Expr> > range;
	range = solver->solver->getRange(Query(state.constraints, bytes));
	info << "\tsize range(MAYBE):[min, max] <--> [" << range.first << ", " << range.second << "]\n";
...
#define XQX_DUMP_PTREE
	...
	if (it==addedStates.end()) {
        ...
        //addbyxqx201511 if addedState to be removed, not swap yet
        searcher->setFlags(false);
...
void Executor::terminateStateOnError(ExecutionState &state,
	...
	if (EmitAllErrors ||
		...
		if (ii.file != "") {
			klee_message("ERROR in state[%d]: %s:%d: %s", state.id,
                    ii.file.c_str(), ii.line, message.c_str());
		} else {
			klee_message("ERROR in state[%d]: (location information missing) %s", 
                    state.id, message.c_str());
		}
		if (!EmitAllErrors)
...
#ifdef XQX_DUMP_LARGE_ALLOC
		if (allocSize>10*1024*1024) {
			klee_xqx_debug( "Large alloc: %u bytes in state[%d].", (unsigned) allocSize, state.id);
			printFileLine(state, state.prevPC);
			std::ostringstream msg;
			state.dumpStack(msg);
			klee_xqx_debug("\n%s", msg.str().c_str());
...
#endif
		MemoryObject *mo = memory->allocate(allocSize, isLocal, false, 
				state.prevPC->inst);
		if (!mo) {
...
#undef xDEBUG
		...
		if (inBounds) {
			...
			if (isWrite) {
                ...
                if( !isa<ConstantExpr>(result) ) {
                    std::ostringstream xinfo;
                    xinfo << "MemoryOperate read result = \n" << result ;
                    klee_xqx_debug("%s in state[%d]",xinfo.str().c_str(), state.id);
                }
#endif
    ...
    std::ostringstream xinfo;
    xinfo << "in memOperate resolve address= " << address << "in state[" << state.id << "]" ;
    klee_xqx_debug(xinfo.str().c_str());
	ResolutionList rl;  
	solver->setTimeout(coreSolverTimeout);
	...
	klee_xqx_debug("ResolutionList size = %d---", rl.size());
	...
	if (unbound) {
		if (incomplete) {
            ...
            klee_xqx_debug("memory error in executeMemoryOperation in state[%d]", unbound->id);
			terminateStateOnError(*unbound,
...
#ifdef XQX_CONCRETE_EXEC
    std::ostringstream msg;
    msg << "read filesize=" << mo->size << ", at addr=" <<std::hex << addressExpr << "\n";
...
#ifdef XQX_CONCRETE_EXEC
        if(i%16==0) msg << "\n";
        msg << std::setw(2) << static_cast<unsigned>(buf) << " ";
#endif
    ...
    if( !enableFork ) {
        ...
        if( !enableFork && isSeeding ) {
            ...
            for (std::vector<SeedInfo>::iterator siit = seeds.begin(), 
                ...
                ref<ConstantExpr> res;
                bool success = 
                    solver->getValue(state, size, res, true);
                assert(success && "FIXME: Unhandled solver failure");
                (void) success;
                uint64_t s = res->getZExtValue();
                klee_xqx_debug("getseedsize=");
                res->dump();

                Expr::Width W = size->getWidth();
                //addConstraint(state, EqExpr::create(size, ConstantExpr::alloc(s,W)));
                executeAlloc(state, res, isLocal, 
                        target, zeroMemory, reallocFrom);
                return true;
...
#ifdef XQX_DEBUG_PRINT_RANGE
						std::pair< ref<Expr>, ref<Expr> > range;
						info << "[min, max] <-->  [" << fixedSize.second->stateRange.first << ", " << fixedSize.second->stateRange.second << "]\n" ;
#endif
						info << "  concretization : " << example << "\n";
						//info << "  unbound example: " << tmp << "\n";
						//info << "Stack: \n";
...
void Executor::CheckMemoryOutofBound(ExecutionState &state, ref<Expr> address, ref<Expr> bytes) 
{
	// we are on an error path (no resolution, multiple resolution, one
	// resolution with out of bounds)
 ...
 * =====================================================================================
 */
bool  Executor::checkForkable ( ExecutionState &state )
{
    Function *f = state.pc->inst->getParent()->getParent();
    if( f )
        return checkForkable(f);
    const Function *cf = state.pc->inst->getParent()->getParent();
    if( cf )
        return checkForkable(cf);
    
    return false;
}

bool Executor::checkForkable(Function *f) 
{
    return kmodule->functionMap[f]->isFocusedFunc;
}

bool  Executor::checkForkable (const Function *f )
{
    enableFork = false;
    for( std::set<const Function*>::iterator it=noForkFuncs.begin(), ie=noForkFuncs.end(); 
            it != ie; it++) {
    ...
    for( std::set<const Function*>::iterator it=forkFuncs.begin(), ie=forkFuncs.end(); 
            it != ie; it++) {
        const Function *func = *it;
        if( f == *it ){
            enableFork = true;
    ...
    return enableFork;
}		/* -----  end of function checkForkable  ----- */

/* 
 * ===  FUNCTION  ======================================================================
 *         Name:  setFocusedFuncs
 *  Description:  set focused functions to forkFuncs by name
 * =====================================================================================
 */
bool Executor::setFocusedFuncs(std::set<std::string> focusedFuncs)
{
    focusedFuncs.insert("__user_main");
    if( focusedFuncs.size() == 0 ) return false;

    //dump focused bbs , addbyxqx
    std::ostream *fbbidFile;
    if( OutputFocusBBs ) {
        fbbidFile =interpreterHandler->openOutputFile("id-fbbs.txt");
    }

    for( std::set<std::string>::iterator it=focusedFuncs.begin(),ie=focusedFuncs.end();
            it!=ie; it++) {
        std::string fName = *it;
        const Function *f = kmodule->module->getFunction(fName);
        if( f ) {
            forkFuncs.insert(f);
            KFunction *kf = kmodule->functionMap[kmodule->module->getFunction(fName)];
            kf->isFocusedFunc = true;
            if(OutputFocusBBs) {
                llvm::Function::iterator fit=kf->function->begin(), fie=kf->function->end();
                for( ; fit!=fie; fit++) {
                    const InstructionInfo &ii = kmodule->infos->getInfo(&fit->front());
                    const std::string &pathFile = ii.file;
                    *fbbidFile << kf->basicBlockID[fit] << "\t\t" << kf->function->getNameStr().c_str() 
            ...
            std::cerr << "a function not found : " << fName << "\n";
        }
    }
    ...
    std::cerr << "set forkFuncs number = " << forkFuncs.size() << "\n";
#ifdef XQX_XPATH
	if (StatsTracker::useStatistics()) {
        std::cerr << "StatsTracker========\n";
