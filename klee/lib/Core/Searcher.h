...
namespace klee {
  ...
  public:
    ...
    virtual void updateEx(ExecutionState *current,
  ...
  class ConcolicDFSSearcher : public Searcher {
    std::vector<ExecutionState*> states;
    bool swapLastState;
  ...
  public:
    ...
    void update(ExecutionState *current,
                const std::set<ExecutionState*> &addedStates,
                const std::set<ExecutionState*> &removedStates);
    void updateEx(ExecutionState *current,
                        const std::vector<ExecutionState*> &xaddedStates,
                        const std::set<ExecutionState*> &addedStates,
                        const std::set<ExecutionState*> &removedStates);
    void setFlags(bool flag) {swapLastState = flag; }
    bool empty() { return states.empty(); }
    void printName(std::ostream &os) {
      os << "ConcolicDFSSearcher\n";
    }
  };
