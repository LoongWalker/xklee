...
ExecutionState &DFSSearcher::selectState() {
    //klee_xqx_debug("dfs: select state[%d]", (*states.back()).id);
  return *states.back();
}
...
ExecutionState &ConcolicDFSSearcher::selectState() {
#ifdef XQX_DEBUG_CONDFS
    klee_xqx_debug("%d state left, ", states.size());
    klee_xqx_debug("Concolicdfs: select state[%d]", (*states.back()).id);
#endif
  return *states.back();
}

void ConcolicDFSSearcher::update(ExecutionState *current,
                         const std::set<ExecutionState*> &addedStates,
                         const std::set<ExecutionState*> &removedStates) {
  //if( addedStates.size() == 2) 
      //states.pop_back();
#ifdef XQX_DEBUG_CONDFS
    klee_xqx_debug("%d state left, addstate size=%d ", states.size(),addedStates.size());
    if(states.size() != 0)
    klee_xqx_debug("Concolicdfs: last state[%d]", (*states.back()).id);
#endif
  states.insert(states.end(),
                addedStates.begin(),
                addedStates.end());
...
#ifdef XQX_DEBUG_CONDFS
      klee_xqx_debug("swap in ConcolicDFSSearcher s[%d] -> s[%d]",
              states[size-1]->id,states[size-2]->id);
#endif
      std::swap(states[size-1],states[size-2]);
      swapLastState = false;
#ifdef XQX_DEBUG_CONDFS
      klee_xqx_debug("after swap ConcolicDFSSearcher s[%d] - s[%d]",
              states[size-1]->id,states[size-2]->id);
#endif
  }
  for (std::set<ExecutionState*>::const_iterator it = removedStates.begin(),
...
void ConcolicDFSSearcher::updateEx(ExecutionState *current,
                         const std::vector<ExecutionState*> &xaddedStates,
                         const std::set<ExecutionState*> &addedStates,
                         const std::set<ExecutionState*> &removedStates) {
 if( xaddedStates.size() == 2) {
      states.pop_back();
#ifdef XQX_DEBUG_CONDFS
      klee_xqx_debug("xxaddedStates[0] id = %d", xaddedStates[0]->id);
      klee_xqx_debug("xxaddedStates[1] id = %d", xaddedStates[1]->id);
#endif
      states.push_back(xaddedStates[0]);
      states.push_back(xaddedStates[1]);
 }
 else {
  states.insert(states.end(),
                addedStates.begin(),
  ...
  for (std::set<ExecutionState*>::const_iterator it = removedStates.begin(),
    ...
    if (es == states.back()) {
      states.pop_back();
#ifdef XQX_DEBUG_CONDFS
      klee_xqx_debug("remove state[%d] in updateEx", es->id);
#endif
    } else {
      bool ok = false;
  ...
  //klee_xqx_debug("updateEx:last state is state[%d]", states.back()->id);
}
