...
public:
  ...
  ref<Expr> getBoundsCheckPointer(ref<Expr> pointer, ref<Expr> bytes) const {
    return getBoundsCheckOffset(getOffsetExpr(pointer), bytes);
  }
  ...
  ref<Expr> getBoundsCheckOffset(ref<Expr> offset, ref<Expr> bytes) const {
      return UltExpr::create(AddExpr::create(offset,bytes),  
                             ConstantExpr::alloc(size + 1, 
                                                 Context::get().getPointerWidth()));
  }
};
