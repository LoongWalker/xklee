...
public:
  ...
  uint64_t stateID;
  bool  enableFork;   // only fork in focused functions
...
private:
   ...
   *  functions need fork
   *-----------------------------------------------------------------------------*/
  std::set<const llvm::Function*> forkFuncs;
  std::set<const llvm::Function*> noForkFuncs;
  ...
  std::vector<ExecutionState*> xaddedStates; //for ConcolicDFSSearcher
  ...
  ///check function if need fork in it
  bool checkForkable(const Function *f);
  bool checkForkable(Function *f);
  bool checkForkable(ExecutionState &state);
...
public:
  ...
  void CheckMemoryOutofBound(ExecutionState &state, ref<Expr> address, ref<Expr> bytes);
  std::string getAddressInfo(ExecutionState &state, ref<Expr> address, ref<Expr> bytes) const;

  // focused functions
  bool setFocusedFuncs(std::set<std::string> focusedFuncs);
  std::set<const Function*> getForkFuncs(){ return forkFuncs;}
  std::ostream *openOutputFile(const std::string &filename){
      return interpreterHandler->openOutputFile(filename);
  }
};
