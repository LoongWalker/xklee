...
namespace klee {
  ...
  inline void StatisticManager::incrementStatistic(Statistic &s, 
    ...
    if (enabled) {
      ...
      if (indexedStats) {
        ...
        indexedStats[s.id*instNum + index] += addend;
        if (contextStats)
  ...
  inline void StatisticManager::setIndexedValue(const Statistic &s, 
    ...
    indexedStats[index + instNum*s.id] = value;
  }
}
