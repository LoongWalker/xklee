...
namespace {
	...
	cl::opt<bool>
		UseLibz("libz", 
				cl::desc("Link with libz.bca"),
				cl::init(false));
	
    cl::opt<std::string>
        SymConfigFile("sym-conf-file",
                cl::desc("a symbolic config file to specify the symbolic data"),
                cl::value_desc("config file"));

    cl::opt<std::string>
        ConFilePath("concolic-file-path",
                cl::desc("a concolic file path as a seed"),
                cl::value_desc("concolic file path"));
	
    cl::opt<unsigned>
        FillFileMethod("fill-method",
                cl::desc("method of fill file, 0-2"),
                cl::init(2));

	cl::opt<bool>
		GenTestcaseOnlyPtrError("gen-ptr-err-only", 
                cl::desc("generate testcase only when ptr.err"),
				cl::init(false));
	
    cl::opt<bool>
        OutputLinkedModule("output-linked-module",
                cl::desc("Write the bitcode for the linked module"),
                cl::init(false));

    cl::opt<bool>
        LinkOnly("linked-only",
                cl::desc("linked modules only"),
                cl::init(false));

    cl::opt<bool>
        RunLinkedBc("run-linked-bc",
                cl::desc("skipped linked other modules"),
                cl::init(false));
}
...
class KleeHandler : public InterpreterHandler {
    ...
    public:
        ...
        static std::ostream *openFile(const std::string &filename, 
...
std::ostream *KleeHandler::openFile(const std::string &filename, std::string newSuffix) {
    std::ios::openmode io_mode = std::ios::out | std::ios::trunc 
        | std::ios::binary;
    std::ostream *f;
    std::string path = filename.substr(0, filename.rfind('.')==std::string::npos ?  
...
void KleeHandler::processTestCase(const ExecutionState &state,
    ...
    // if not ptr.err, we need not generate testcase and pc, but cov.
    bool isGenAll = true;
    if( GenTestcaseOnlyPtrError && errorSuffix != "ptr.err" )
        isGenAll = false;
    ...
    if (!NoOutput) {
        ...
        if (isGenAll && (errorMessage || WritePCs) )  {
...
static int initEnv(Module *mainModule) {
    ...
    if( !mainFn ) {
        klee_error("not find main function in mainModule");
    }
...
int main(int argc, char **argv, char **envp) {  
...
#ifdef XQX_XPATH
    //get functions info in mainModule
    FunctionAnalysis *fa = new FunctionAnalysis();
    fa->setOutputDir(OutputDir, InputFile);
    fa->getFuncsInModule( mainModule, "mainModule.txt");
#endif
    ...
    if( RunLinkedBc ) goto LabelRunLinkedBc;
...
#ifdef XQX_XPATH
    fa->getFuncsInModule( mainModule, "initEnv-mainModule.txt",true);
#endif
  ...
  if( LibRuntime ) {
      ...
      for(libs_it=LibPathFile.begin(), libs_ie=LibPathFile.end();
              libs_it!=libs_ie; ++libs_it) {
          std::string libFile = libs_it->c_str();
#ifdef XQX_XPATH
          //fa->getFuncsInModule(LibPathFile, "xlib.txt");
          llvm::sys::Path xlibBCA(libFile);
          fa->getFuncsInArchive(xlibBCA, "xlib.txt", true);
#endif
          klee_message("XQX: using lib %s", libFile.c_str());
          mainModule = klee::linkWithLibrary(mainModule, libFile.c_str());
    ...
    switch (Libc) {
        ...
        case UcLibc:
#ifdef XQX_XPATH
                       llvm::sys::Path uclibcBCA(LibraryDir);
                       uclibcBCA.appendComponent(KLEE_UCLIBC_BCA_NAME);
                       fa->getFuncsInArchive(uclibcBCA, 
                               "uclibc.txt");
#endif
    ...
    if (UseLibz) {
        llvm::sys::Path Path(Opts.LibraryDir);
        Path.appendComponent("libz.bca");
        klee_message("NOTE: Using model: %s", Path.c_str());
        mainModule = klee::linkWithLibrary(mainModule, Path.c_str());
        assert(mainModule && "unable to link with simple model");
    }  

    if (OutputLinkedModule) {
        std::ostream *f = KleeHandler::openFile(InputFile.c_str(),"-linked.bc");
...
LabelRunLinkedBc:
  ...
  StripFunc64(mainModule);
	...
	// addbyxqx201504  for parsing the symbolic configure file
	// file format, off-len,off-len,....
	int nParts=0;
	//int nOff[1024],nLen[1024];
	std::vector<std::string> off_vect, len_vect;
	std::string sLine;

	if( SymConfigFile != "" ) {
		std::ifstream ifs(SymConfigFile.c_str(), std::ios::in);
		...
		if(!ifs.good()) {
			klee_message("open configure file error: %s", SymConfigFile.c_str());
			return -1;
		}
		...
		while(getline(ifs, sLine))
		{
			...
			while(( pos = sLine.find(",")) != std::string::npos) {
				...
				if( p1 == std::string::npos ){
					klee_error("symconfigfile parsing error!");
					return -1;
				}
				off = token.substr(0,p1);
				len = token.substr(p1+1, token.length());
				//std::cout <<"off="<< off << ",len="<<len << "\n";
				
				off_vect.push_back(off);
				len_vect.push_back(len);
				nParts++;
	...
	// add addtional args for concolic file
	if( nParts != 0)  {
		klee_message("[xqx]klee addtional args: -----------------------");
		int pArgcNew;
		char **pArgvNew;
		pArgcNew = pArgc + 4 + nParts*3;
		pArgvNew = new char *[pArgcNew];
		memcpy(pArgvNew, pArgv, pArgc*sizeof(char*));
		std::string xconfile = "--xqx-confile";
		std::string xpath = ConFilePath;
		std::string xnum = SI2Str(nParts);
		std::string xmethod = SI2Str(FillFileMethod);
		std::vector<std::string> tmpArgv;
		tmpArgv.push_back(xconfile);
		tmpArgv.push_back(xpath);
		tmpArgv.push_back(xnum);
		tmpArgv.push_back(xmethod);

		for (unsigned i=0; i<tmpArgv.size(); i++) {
			std::string &arg = tmpArgv[i];
			unsigned size = arg.size() + 1;
			char *pArg = new char[size];
			std::copy(arg.begin(), arg.end(), pArg);
			pArg[size - 1] = 0;
			pArgvNew[pArgc] = pArg;
			klee_message("[xqx] tmpArgv[%d]: %s ", i , pArgvNew[pArgc]);
			pArgc++;
		}

		for (unsigned j=0; j<nParts; j++) {
			std::string xofflen = "--xqx-offlen";
			tmpArgv.clear();
			tmpArgv.push_back(xofflen);
			tmpArgv.push_back(off_vect[j]);
			tmpArgv.push_back(len_vect[j]);
			for (unsigned i=0; i<tmpArgv.size(); i++) {
				std::string &arg = tmpArgv[i];
				unsigned size = arg.size() + 1;
				char *pArg = new char[size];
...
#ifdef XQX_XPATH
    fa->getFuncsInModule( finalModule, "finalModule.txt");
    fa->logFocusedFuncs("focusedFuncs.txt");
    interpreter->setFocusedFuncs(fa->getFocusedFuncs());
    CGAnalysis *cg = new CGAnalysis(mainModule);
    cg->importFocusedFuncs((fa->getFocusedFuncs()));
    cg->setFS2CGPass(fa->openOutputFile("callgraph.dot"));
    //cg->genCallGraph();
#endif
